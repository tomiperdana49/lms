...
const providedData = `21/01/2026 16:11:29	adhitya@nusa.net.id	How To Win Friends & Influence People	100.000
...
21/01/2026 9:20:07	harianto@nusa.net.id	5 Kesalahan Sales dan Solusinya	100.000
...
async function verifyAllData() {
    try {
        const lines = providedData.trim().split('\n');
        const missing = [];
        let foundCount = 0;

        console.log(`Checking ${lines.length} entries...`);

        for (const line of lines) {
            const [date, email, title, incentive] = line.split('\t');
            if (!email || !title) continue;

            // Check if matching record exists (Approved)
            const [rows] = await pool.query(
                `SELECT rl.id 
                 FROM reading_logs rl 
                 JOIN users u ON rl.employee_id = u.employee_id 
                 WHERE u.email = ? AND LOWER(TRIM(rl.title)) = LOWER(TRIM(?)) AND rl.hr_approval_status = 'Approved'`,
                [email.trim().toLowerCase(), title.trim()]
            );
        ...
        console.log(`- Found (Approved): ${foundCount}`);
        console.log(`- Missing/Not Approved: ${missing.length}`);

        if (missing.length > 0) {
            console.log('\nMissing Entries:');
            missing.forEach(m => console.log(`${m.email} - ${m.title}`));
        }
