...
const PopupNotification = ({ type, message, isOpen, onClose }: PopupNotificationProps) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all duration-300 ${isOpen ? 'bg-slate-900/50 backdrop-blur-sm opacity-100' : 'bg-transparent opacity-0 pointer-events-none'}`}>
            ...
            >
                <div className={`h-2 w-full ${isSuccess ? 'bg-teal-500' : isError ? 'bg-red-500' : isWarning ? 'bg-amber-500' : 'bg-blue-500'}`} />
                ...
                <div className="p-6 relative">
                    ...
                    <div className="flex flex-col items-center text-center space-y-4">
                        <div className={`p-4 rounded-full ${isSuccess ? 'bg-teal-50 text-teal-500' : isError ? 'bg-red-50 text-red-500' : isWarning ? 'bg-amber-50 text-amber-500' : 'bg-blue-50 text-blue-500'}`}>
                            {isSuccess ? <CheckCircle size={40} strokeWidth={2.5} /> : isError ? <XCircle size={40} strokeWidth={2.5} /> : isWarning ? <AlertTriangle size={40} strokeWidth={2.5} /> : <Info size={40} strokeWidth={2.5} />}
                        </div>

                        <div>
                            <h3 className={`text-xl font-bold mb-1 ${isSuccess ? 'text-slate-800' : isError ? 'text-red-600' : isWarning ? 'text-amber-600' : 'text-slate-800'}`}>
                                {isSuccess ? t('success') : isError ? t('error') : isWarning ? t('warning') : t('information')}
                            </h3>
                        ...
                        >
                            {isSuccess ? t('continue') : isError ? t('tryAgain') : isWarning ? t('understood') : t('close')}
                        </button>
                    </div>
                </div>
            </div>
